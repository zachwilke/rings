...
```bash
cargo build --release --target x86_64-unknown-linux-musl   # static, 598,752 bytes
cargo build --release                                      # glibc,  493,464 bytes
```
...
| `x` — confirm delete · `e` — export CSV · `?` — help · `q` — quit | |
