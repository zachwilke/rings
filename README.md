...
The one-liner lives on `main` (so the platform map stays current) and fetches the latest published release, not a pinned tag. Pin with `RING_VERSION=v0.3.1`; install to a chosen directory with `RING_PREFIX`. Unix needs `curl` or `wget`, plus `xz` (`apt install xz-utils` / `brew install xz`).
...
```bash
curl -fsSL -o rings.deb https://github.com/zachwilke/rings/releases/download/v0.3.1/rings_0.3.1_amd64.deb
sudo apt install ./rings.deb
```

arm64: `rings_0.3.1_arm64.deb`. armhf: `rings_0.3.1_armhf.deb`.
...
curl -fsSL https://github.com/zachwilke/rings/releases/download/v0.3.1/rings-aarch64-linux-musl.xz | xz -d > rings
# 32-bit Raspberry Pi OS (Pi 2/3/4)
# curl -fsSL https://github.com/zachwilke/rings/releases/download/v0.3.1/rings-armv7-linux-musleabihf.xz | xz -d > rings
# Pi 1 / Zero (armv6)
# curl -fsSL https://github.com/zachwilke/rings/releases/download/v0.3.1/rings-arm-linux-musleabihf.xz | xz -d > rings
chmod +x rings
sudo ./rings /
```
...
```bash
# Apple Silicon
curl -fsSL https://github.com/zachwilke/rings/releases/download/v0.3.1/rings-aarch64-apple-darwin.xz | xz -d > rings
...
```powershell
irm https://github.com/zachwilke/rings/releases/download/v0.3.1/rings-x86_64-pc-windows-msvc.exe.zip -OutFile rings.zip
